...
class ProfileController extends Controller
{
    ...
    public function getProfileData(Request $request)
    {
        ...
        $cacheKey = 'profile:data:' . $username . ':viewer:' . me()->id;
        $cached = Cache::remember($cacheKey, 30, function () use ($username) {
            $profileData = User::activeByUsername($username)
                ->with([
                    'privacySettings',
                    'businessAccount',
                ])
                ->withCount([
                    'posts as publications_count',
                    'followers as followers_count' => function ($query) {
                        $query->where('status', FollowStatus::FOLLOWING);
                    },
                    'followings as following_count' => function ($query) {
                        $query->where('status', FollowStatus::FOLLOWING);
                    },
                ])
                ->first();
            ...
            if (me()->isBlockedWith($profileData)) {
                return '__blocked__';
            }

            return ProfileResource::make($profileData)->resolve();
        });
        ...
        if ($cached === '__blocked__') {
            return $this->responseError([
                ...
                'errors' => [
                    'id' => ['This profile is not available.']
                ]
            ], 403);
        }
    ...
    public function getProfileDetails(Request $request)
    {
        ...
        $cacheKey = 'profile:details:' . $profileId . ':viewer:' . me()->id;
        $cached = Cache::remember($cacheKey, 30, function () use ($profileId) {
    ...
    {   
        ...
        if($contentType == 'posts' || $contentType == 'media') {
            $profilePosts = $profileData->posts()->timelineFormatPosts()
                ->excludeBlockedUsers(me()->id)
