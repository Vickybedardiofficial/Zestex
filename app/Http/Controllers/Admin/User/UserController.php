...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::whereIn('type', [UserType::AUTHOR->value, UserType::READER->value])
        ->when((! empty($this->filters['search'])), function ($query) {
            ...
            $query->where('type', ($this->filters['type'] == 'author') ? UserType::AUTHOR->value : UserType::READER->value);
        })->latest()->paginate(10);
    ...
    {
        return User::active()
            ->whereIn('type', [UserType::AUTHOR->value, UserType::READER->value])
            ->findOrFail($userId);
    }
}
