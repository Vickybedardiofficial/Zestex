...
class ConfigController extends Controller
{
    ...
    {
        $verificationRequired = AdminSetting::where('key', 'user_email_verification')->value('value');
        // Default to true if not set
        $verificationRequired = $verificationRequired === null ? 'true' : $verificationRequired;
    ...
    public function updateVerification(Request $request)
    {
        $data = $request->validate([
            'user_email_verification' => 'required|in:true,false'
        ]);

        AdminSetting::updateOrCreate(
            ['key' => 'user_email_verification'],
            ['value' => $data['user_email_verification'], 'type' => 'boolean']
        );

        return redirect()->back()->with('flashMessage', (new Flash(content: 'Verification settings updated successfully.'))->get());
    }
