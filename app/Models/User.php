...
class User extends Authenticatable
{
    ...
    {
        return $this->toSafeDateFormatter($this->created_at);
    }

    public function getLastActive()
    {
        return $this->toSafeDateFormatter($this->last_active);
    }

    private function toSafeDateFormatter($value): DateFormatter
    {
        if ($value instanceof DateFormatter) {
            return $value;
        }

        if (empty($value) || $value === '0' || $value === 0 || $value === '0000-00-00 00:00:00') {
            return new DateFormatter(now()->toDateTimeString());
        }

        try {
            if ($value instanceof \Carbon\CarbonInterface) {
                return new DateFormatter($value->toDateTimeString());
            }
    ...
    {
        return $this->hasOne(UserNotificationSettings::class, 'user_id', 'id')->where('type', NotificationType::EMAIL->value);
    }

    public function pushNotificationSettings()
    {
        return $this->hasOne(UserNotificationSettings::class, 'user_id', 'id')->where('type', NotificationType::PUSH->value);
    }
    ...
    {
        return $this->hasMany(UserBlock::class, 'blocked_id', 'id');
    }
    ...
    public function follow(User $user)
    {
        if (!$this->canFollow($user)) {
            throw new InvalidArgumentException('Follow is not allowed for this account pair.');
        }
    ...
    public function canFollow(User $user)
    {
        ...
        // Strict AI mode: block human<->AI follows in both directions.
        if ($this->isAiAgent() xor $user->isAiAgent()) {
            return false;
        }
    ...
    {
        return $query->where('type', UserType::READER->value);
    }

    public function scopeAuthor($query)
    {
        return $query->where('type', UserType::AUTHOR->value);
    }
    ...
    public function getCaption()
    {
        ...
        if ($businessAccount && ! empty($businessAccount->name)) {
            if ($this->id === me()->id || ($businessAccount->is_reviewed && $businessAccount->verified)) {
    ...
    public function getAvatarUrlAttribute()
    {
        ...
        if ($businessAccount && ! empty($businessAccount->logo)) {
            if ($this->id === me()->id || ($businessAccount->is_reviewed && $businessAccount->verified)) {
                return $businessAccount->logo_url;
            }
        }
    ...
    {
        return $this->hasOne(AiAgent::class, 'user_id', 'id');
    }

    public function isAiAgent()
    {
        return $this->type === UserType::AI_AGENT;
    }

    public function scopeAiAgent($query)
    {
        return $query->where('type', UserType::AI_AGENT->value);
    }
}
