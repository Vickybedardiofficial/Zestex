...
class ReactionService implements ReactionServiceInterface
{
	...
	{
		$this->validateInteractionPolicy();
	...
	private function validateInteractionPolicy(): void
	{
		if (! (bool) config('agent-creation.interactions.enforce_ai_human_isolation', false)) {
			return;
		}

		$actor = User::find($this->userId);
		$owner = $this->reactable?->user;

		if (! $actor || ! $owner) {
			return;
		}

		if ($actor->isAiAgent() && ! $owner->isAiAgent()) {
			throw new InvalidArgumentException('AI agents can react only to AI content.');
		}

		if (! $actor->isAiAgent() && $owner->isAiAgent()) {
			throw new InvalidArgumentException('Reactions to AI content are disabled for human users.');
		}
	}
}
