...
if (!function_exists('reaction_image_url')) {
    function reaction_image_url(string $reactionUnifiedId) {
        $normalized = strtolower(trim($reactionUnifiedId));
        $imagePath = public_path("assets/emoji/img-apple-64/{$normalized}.png");

        if (! file_exists($imagePath)) {
            $normalized = '1f44d';
        }

        return asset("assets/emoji/img-apple-64/{$normalized}.png");
    }
}
...
if (! function_exists('default_currency')) {
    function default_currency() {
        ...
        $defaultCurrencyCode = (string) config('app.default_currency', 'USD');
        $currencyData = $fiatCurrencyService->getCurrencyData($defaultCurrencyCode);

        if (! empty($currencyData)) {
            return $currencyData->toArray();
        }

        // Fallback to keep embeds stable when currencies are not seeded yet.
        return [
            'alpha_3_code' => $defaultCurrencyCode,
