			...
			return {
				...
				toggleVideosVolume: () => {
                    if (localStorage.getItem('stories_videos_muted')) {
						...
						ZESTEXEventBus.emit('story:unmute');
                    }
                    else{
                        localStorage.setItem('stories_videos_muted', 1);
                        state.isMuted = true;

						ZESTEXEventBus.emit('story:mute');
                    }
                },
				deleteStory: () => {
					ZESTEXEventBus.emit('story:delete', playerState.frameData.id);
				},
				showViews: () => {
                    ZESTEXEventBus.emit('story:show-views');
                },
				openMenu: () => {
                    state.mainMenu.open();
				...
				leaveStories: () => {
					ZESTEXEventBus.emit('story:leave');
				}
			};
		},
		components: {
