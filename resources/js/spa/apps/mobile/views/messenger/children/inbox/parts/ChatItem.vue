		...
		setup: function(props) {
			...
			onUnmounted(() => {
                if(window.ZESTEXBRConnected) {
					ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).stopListeningForWhisper(BRD.getEvent('CHAT_MESSAGE_TYPING'));
				}
			});

			onMounted(() => {
				if(window.ZESTEXBRConnected) {
					ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).listenForWhisper(BRD.getEvent('CHAT_MESSAGE_TYPING'), function (event) {
