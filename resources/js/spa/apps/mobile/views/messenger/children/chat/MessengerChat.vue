		...
		setup: function() {
			...
			onUnmounted(() => {
                if(window.ZESTEXBRConnected) {
					...
					ZESTEXBRD.private(chatChannel.value).stopListeningForWhisper(BRD.getEvent('CHAT_MESSAGE_TYPING'));
                }
            });

			const stopListenEventInChat = (eventName) => {
                ZESTEXBRD.private(chatChannel.value).stopListening(eventName);
            }

            const listenEventInChat = (eventName, callback) => {
                ZESTEXBRD.private(chatChannel.value).listen(eventName, callback);
            }

			const whisperToChat = (isTyping) => {
                ZESTEXBRD.private(chatChannel.value).whisper(BRD.getEvent('CHAT_MESSAGE_TYPING'), {
                    data: {
                        user: {
			...
			onMounted(async function() {
                try {
					...
					if(window.ZESTEXBRConnected) {
						...
						ZESTEXBRD.private(chatChannel.value).listenForWhisper(BRD.getEvent('CHAT_MESSAGE_TYPING'), function (event) {
