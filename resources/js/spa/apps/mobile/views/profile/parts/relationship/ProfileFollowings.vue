	...
	<ActionSheet>
		<div class="h-full flex flex-col">
			<div class="px-4 pb-4 border-b border-b-bord-pr text-center">
				<SheetTitle v-bind:title="$t('labels.following_count', profileData.following_count.raw) + ' ' + profileData.following_count.formatted"></SheetTitle>
			</div>
			<div class="flex-1 overflow-y-auto">
				...
				<template v-else-if="people.length === 0">
					<div class="py-42">
						<TimelineEmptyState v-bind:desc="$t('labels.user_no_following', { name: profileData.name })"></TimelineEmptyState>
					</div>
				</template>
				<template v-else>
