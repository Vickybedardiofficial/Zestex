		...
		<div class="flex-1 overflow-y-auto">
			<div class="flex flex-col h-full">
				<div class="flex-1">
					...
					<div v-if="state.mentions.length" class="px-4 pb-3 -mt-1">
						<div class="rounded-xl border border-bord-pr popup-background-tr max-h-56 overflow-y-auto">
							<button
				...
				<div class="shrink-0">
					...
					<div class="px-4 mb-4 relative" v-if="quotedPost">
						<div class="absolute -top-2 -right-1 z-10">
							<div class="bg-bg-pr rounded-full shadow-sm">
								<PrimaryIconButton
									...
									class="border border-bord-card"
								></PrimaryIconButton>
							</div>
						</div>
						<PublicationQuote v-bind:quotedPost="quotedPost"></PublicationQuote>
					</div>
				</div>
			</div>
		</div>
	...
	export default defineComponent({
		setup: function() {
			...
			const getFormSubmitData = () => {
				...
				if (quotedPost.value) {
					formData.quoted_post_id = quotedPost.value.id;
				}
			...
			onMounted(async function() {
				if (route.query.quote_post_id) {
					postEditorStore.setQuotePostId(route.query.quote_post_id);
				}
			...
			const uploadMedia = (mediaFile, type = 'image') => {
                ...
                ZESTEXAPI().postEditor().with(formData).withHeaders({
			...
			const submitForm = async () => {
                ...
                await ZESTEXAPI().postEditor().with(getFormSubmitData()).sendTo('create').then((response) => {
			...
			return {
				...
				textInputHandler: function() {
					...
					const mentionMatch = matchMention(contentInput.value);
					if(mentionMatch) {
				...
				createPoll: () => {
					ZESTEXAPI().postEditor().sendTo('poll/create').then((response) => {
						postEditorStore.fetchDraftPost();
					}).catch((error) => {
						validatePost(error.response.data.message);
					});
				...
				selectMention: (username) => {
					const mentionMatch = matchMention(contentInput.value);
					if(mentionMatch) {
						postData.value.content = completeText(contentInput.value, {
