...
const useChatStore = defineStore('mobile_chats_chat', {
	...
	actions: {
		fetchChatData: async function(chatId) {
			await ZESTEXAPI().messenger().getFrom(`chat/${chatId}`).then((response) => {
		...
		fetchChatParticipants: async function() {
			await ZESTEXAPI().messenger().getFrom(`chat/${this.chatId}/participants`).then((response) => {
		...
		fetchChatMessages: async function() {
			await ZESTEXAPI().messenger().getFrom(`chat/${this.chatId}/messages`).then((response) => {
		...
		markMessagesAsRead: function() {
			ZESTEXAPI().messenger().getFrom(`chat/${this.chatId}/read`).catch(function(error) {
				alert(error.response.data.message);
			});
		},
		...
		clearChat: async function() {
			await ZESTEXAPI().messenger().delete(`chat/${this.chatId}/clear`).then((response) => {
				this.chatMessages = [];
			}).catch((error) => {
				if(error.response) {
					throw new Error(error.response.data.message);
		...
		deleteChat: async function() {
			await ZESTEXAPI().messenger().delete(`chat/${this.chatId}/delete`).then((response) => {
				this.chatMessages = [];
