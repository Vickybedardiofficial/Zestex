...
const useStoriesStore = defineStore('mobile_stories_store', {
    ...
    actions: {
        fetchStoriesFeed: async function() {
            await ZESTEXAPI().stories().getFrom('feed').then((response) => {
        ...
        fetchStory: async function(storyUUID) {
            await ZESTEXAPI().stories().getFrom(`stories/${storyUUID}`).then((response) => {
        ...
        fetchAndReturnStoryViews: async function(frameId) {
            return await ZESTEXAPI().stories().getFrom(`views/${frameId}`).then((response) => {
