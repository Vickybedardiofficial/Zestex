<template>
    <div class="base-publication min-h-40 border-b border-b-bord-tr">
        <div class="pt-4 max-w-full">
            ...
            <div class="max-w-full">
                ...
                <div class="px-4" v-if="!isPromoted && postReactions.length">
                    <ReactionsViewer v-on:add="addReaction" v-bind:reactions="postReactions"></ReactionsViewer>
                </div>
                <div v-if="!isPromoted" class="pr-4 pl-3 mb-3">
                    <div class="flex items-center gap-1.5">
                        <div class="shrink-0 relative leading-zero">
                            <PrimaryIconButton v-on:click.stop="openReactionsPicker" buttonColor="text-lab-pr2" iconSize="6" iconName="heart-rounded" iconType="line"></PrimaryIconButton>
                        </div>
                        ...
                        <div class="shrink-0 leading-zero relative">
                            <div class="inline-flex items-center">
                                <PrimaryIconButton
                                    v-on:click.stop="state.repostMenu.open"
                                ...
                                <span v-if="postData.quotes_count && postData.quotes_count.raw" class="text-lab-pr2 text-sm font-medium">
                ...
                <div v-else class="pr-4 pl-3 mb-3">
    ...
    <PublicationReactions v-if="!isPromoted && state.isReactionPickerOpen"
        v-on:add="addReaction"
    v-on:close="closeReactionsPicker"></PublicationReactions>

    <PublicationShare v-if="!isPromoted && state.shareMenu.status"
        v-bind:postLink="postLink"
        v-on:close="state.shareMenu.close"></PublicationShare>

    <PublicationComments v-bind:postData="postData" v-if="!isPromoted && state.commentsMenu.status" v-on:close="state.commentsMenu.close"></PublicationComments>

    <ActionSheet v-if="!isPromoted && state.repostMenu.status" v-on:close="state.repostMenu.close" v-bind:isMuted="true">
        <div v-on:click.stop="state.repostMenu.close" class="h-full overflow-y-auto">
            <div class="mb-4">
                <ActionSheetGroup>
                    <ActionSheetItem
                        v-on:click="toggleRepost"
                        iconName="repost-01"
                    v-bind:textLabel="postData.meta.activity.reposted ? $t('dd.post.undo_repost') : $t('dd.post.repost')"></ActionSheetItem>

                    <ActionSheetItem
                        v-on:click="quotePost"
                        iconName="pencil-line"
                    v-bind:textLabel="$t('dd.post.quote_post')"></ActionSheetItem>
                </ActionSheetGroup>
            </div>
        </div>
    </ActionSheet>

    <ActionSheet v-if="!isPromoted && state.mainMenu.status" v-on:close="state.mainMenu.close" v-bind:isMuted="true">
        <div v-on:click.stop="state.mainMenu.close" class="h-full overflow-y-auto">
            ...
            <div class="mb-4">
                <ActionSheetGroup>
                    <ActionSheetItem
                        v-on:click="openReactionsPicker"
