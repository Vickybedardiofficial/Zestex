...
<script>
	...
	export default defineComponent({
		setup: function() {
			...
			onMounted(() => {
				ZESTEXEventBus.on('report:open', openReportModal);
				ZESTEXEventBus.on('report:close', closeReportModal);
			});

			onUnmounted(() => {
				ZESTEXEventBus.off('report:open', openReportModal);
				ZESTEXEventBus.off('report:close', closeReportModal);
			});
