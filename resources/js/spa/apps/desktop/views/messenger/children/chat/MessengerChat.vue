    ...
    export default defineComponent({
        setup: function(props, context) {
                ...
                try {
                    ...
                    if(window.ZESTEXBRConnected) {
                        listenEventInChat(BRD.getEvent('CHAT_MESSAGE_RECEIVED'), function (event) {
                            ...
                            if(! isSender) {
                                ZESTEXSounds.activeChatMessageReceived();
                                chatStore.markMessagesAsRead();
                            }
                        });
            ...
            const stopListenEventInChat = (eventName) => {
                ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).stopListening(eventName);
            }

            const listenEventInChat = (eventName, callback) => {
                ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).listen(eventName, callback);
            }

            const stopListeningForWhisperInChat = (whisperEvent) => {
                ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).stopListeningForWhisper(whisperEvent);
            }

            const listenWhisperInChat = (whisperEvent, callback) => {
                ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).listenForWhisper(whisperEvent, callback);
            }

            const whisperToChat = (whisperEvent, eventData) => {
                ZESTEXBRD.private(BRD.getChannel('CHAT', [chatData.value.chat_id])).whisper(whisperEvent, eventData);
            }
            ...
            return {
                ...
                handleMessageDelete: (messageData) => {
                    ...
                    ZESTEXEventBus.emit('confirmation-modal:open', modalData);
                },
