					...
					<div class="w-96">
						...
						<div class="block">
							...
							<div class="block">
								...
								<div v-if="isNotDeleted && hasContent" class="text-par-l markdown-text leading-snug font-medium text-lab-pr2" v-html="$mdInline(messageContent)"></div>
								<div v-else-if="!isNotDeleted" class="flex mt-1">
									<p class="text-lab-sc text-par-m leading-snug">
										{{ $t('chat.message_is_deleted') }}
									</p>
								</div>
								<div v-if="isNotDeleted && hasAttachments" class="mt-2 grid gap-2">
									<div v-for="attachment in attachments" v-bind:key="attachment.id" class="max-w-full">
										<template v-if="attachment.type === 'image'">
											<img v-bind:src="attachment.preview_url" class="rounded-lg max-w-full" />
										</template>
										<template v-else-if="attachment.type === 'video'">
											<video class="rounded-lg max-w-full" controls v-bind:src="attachment.url"></video>
										</template>
										<template v-else-if="attachment.type === 'audio'">
											<audio class="w-full" controls v-bind:src="attachment.url"></audio>
										</template>
										<template v-else>
											<a v-bind:href="attachment.url" class="inline-flex items-center gap-2 text-par-m text-brand-900 underline" target="_blank">
	...
	export default defineComponent({
		...
		setup: function (props, context) {
			...
			return {
				...
				hasContent: computed(() => {
					return (messageData.value.content || '').trim().length > 0;
				}),
				attachments: computed(() => {
					return messageData.value.relations?.attachments || [];
				}),
				hasAttachments: computed(() => {
					return (messageData.value.relations?.attachments || []).length > 0;
				}),
				...
				translate: async () => {
                    ...
                    const translatedText = await ZESTEXTranslator().translate(messageData.value.content);
