...
<script>
    ...
    import TodaysNews from '@D/components/news/TodaysNews.vue';
    ...
    export default defineComponent({
        setup: function() {
            ...
            onMounted(async () => {
                ...
                if(window.ZESTEXBRD) {
                    ZESTEXBRD.private(BRD.getChannel('AUTH_USER', [authStore.userData.id]))
                        .listen(BRD.getEvent('TIMELINE_COMMENT_CREATED'), function(event) {
                            timelineStore.setPostCommentData(event);
                        });
                }
            });
            ...
            onUnmounted(() => {
                ...
                if(window.ZESTEXBRD) {
                    ZESTEXBRD.private(BRD.getChannel('AUTH_USER', [authStore.userData.id]))
                        .stopListening(BRD.getEvent('TIMELINE_COMMENT_CREATED'));
                }
			});
            ...
            return {
                ...
                handlePostDelete: (postData) => {
                    postDeleter(postData, (postId) => {
                        ZESTEXEventBus.emit('timeline:post-deleted', postId);
                    });
                },
                applyTimelineUpdate: () => {
