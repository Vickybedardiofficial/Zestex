        ...
        <template v-slot:content>
            <TimelineContainer>
                ...
                <div v-else class="block">
                    ...
                    <Border></Border>
                    <div v-if="! state.isLoading && isAuthenticated" class="sticky top-0 bg-bg-pr z-10">
    ...
    export default defineComponent({
        setup: function() {
                ...
                if (postComments.value.length && ! state.noMoreComments && ! state.isLoadingComments) {
                    ...
                    if (cursorId) {
                        ...
                        const endpoint = authStore.authCheck
                            ? `post/${route.params.hash_id}/comments`
                            : `timeline/publication/${route.params.hash_id}/comments`;

                        await api.params({ 
                            cursor: cursorId
                        }).getFrom(endpoint).then(function(response) {
            ...
            onMounted(() => {
                const api = authStore.authCheck ? ZESTEXAPI().userTimeline() : ZESTEXAPI().publicApi();
                const endpoint = authStore.authCheck
                    ? `post/${route.params.hash_id}`
                    : `timeline/publication/${route.params.hash_id}`;

                api.getFrom(endpoint).then(function(response) {
            ...
            return {
                ...
                isAuthenticated: computed(() => authStore.authCheck),
                ...
                handleCommentReply: (commentId) => {
                    ...
                    if(commentData) {
                        ZESTEXEventBus.emit('publication-comment:reply', {
                ...
                handlePostDelete: (postData) => {
                    ZESTEXEventBus.emit('timeline:post-deleted', postData.id);
