...
<script>
    ...
    export default defineComponent({
        setup: function() {
            ...
            onMounted(async () => {
                await ZESTEXAPI().userSettings().getFrom('privacy/settings').then((response) => {
            ...
            const submitForm = async () => {
                if (state.isSubmitting === false) {
                    ...
                    await ZESTEXAPI().userSettings().with(formData.value).putTo('privacy/update').then((response) => {
