    ...
    <div v-if="! state.isLoading" class="block">
        ...
        <form v-on:submit.prevent="submitForm">
            ...
            <div class="block mb-16">
                <PrimaryPillButton v-bind:loading="state.isSubmitting" buttonType="submit" v-bind:buttonText="$t('labels.save_changes')"></PrimaryPillButton>
            </div>
        </form>
    </div>
...
<script>
    ...
    export default defineComponent({
        setup: function() {
            ...
            onMounted(async () => {
                await ZESTEXAPI().userSettings().getFrom('password/settings').then((response) => {
            ...
            return {
                ...
                generatePassword: async () => {
                    ...
                    await ZESTEXAPI().userSettings().getFrom('password/generate').then((response) => {
                ...
                submitForm: async () => {
                    if (state.isSubmitting === false) {
                        ...
                        await ZESTEXAPI().userSettings().with(formData.value).putTo('password/update').then((response) => {
