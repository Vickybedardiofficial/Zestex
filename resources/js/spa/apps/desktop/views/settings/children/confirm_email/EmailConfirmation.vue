...
<script>
    ...
    export default defineComponent({
        setup: function() {
            ...
            onMounted(async () => {
                await ZESTEXAPI().userSettings().getFrom('email/check').then((response) => {
                    ...
                    if (error.response) {
                        router.push({ name: 'settings_email' });
                    }
                });
            });
            ...
            return {
                ...
                resendCode: async () => {
                    if (state.isSubmitting === false) {
                        ...
                        await ZESTEXAPI().userSettings().sendTo('email/update/resend').then((response) => {

                            toastSuccess(__t('toast.confirmation_code_resent'));
                ...
                confirmCode: async () => {
                    if (state.isSubmitting === false) {
                        ...
                        await ZESTEXAPI().userSettings().with(formData).sendTo('email/update/confirm').then((response) => {
                            toastSuccess(__t('toast.email_updated'));
