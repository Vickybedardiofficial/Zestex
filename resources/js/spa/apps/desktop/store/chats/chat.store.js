...
const useChatStore = defineStore('chats_chat', {
	...
	actions: {
		fetchChatData: async function(chatId) {
			...
			await ZESTEXAPI().messenger().getFrom(`chat/${chatId}`).then(function(response) {
		...
		fetchChatParticipants: async function() {
			...
			await ZESTEXAPI().messenger().getFrom(`chat/${state.chatId}/participants`).then(function(response) {
		...
		markMessagesAsRead: function() {
			...
			ZESTEXAPI().messenger().getFrom(`chat/${state.chatId}/read`).catch(function(error) {
		...
		fetchChatMessages: async function() {
			...
			await ZESTEXAPI().messenger().getFrom(`chat/${state.chatId}/messages`).then(function(response) {
		...
		archiveChat: async function(chatId) {
			await ZESTEXAPI().messenger().delete(`chat/${chatId}/archive`).then((response) => {
		...
		unarchiveChat: async function(chatId) {
			await ZESTEXAPI().messenger().delete(`chat/${chatId}/unarchive`).then((response) => {
		...
		clearChatConversation: async function() {
			...
			await ZESTEXAPI().messenger().delete(`chat/${state.chatId}/clear`).then(function(response) {
				state.chatMessages = [];
			}).catch(function(error) {
				if(error.response) {
					throw new Error(error.response.data.message);
		...
		deleteChat: async function() {
			...
			await ZESTEXAPI().messenger().delete(`chat/${state.chatId}/delete`).then(function(response) {
				state.chatMessages = [];
