...
const useMarketStore = defineStore('marketplace', {
    ...
    actions: {
		...
		makeLoadRequest: async function () {
			return await ZESTEXAPI().marketplace().with({
				filter: this.filter
			}).sendTo('products');
		},
		...
		fetchBookmarkedProducts: async function() {
			await ZESTEXAPI().marketplace().getFrom('bookmarks').then((response) => {
		...
		fetchProduct: async function(id) {
			...
			await ZESTEXAPI().marketplace().getFrom(`products/${id}`).then((response) => {
				this.product = response.data.data;
			}).catch((error) => {});
		},
		fetchCategories: async function() {
			if(! this.categories.length) {
				await ZESTEXAPI().marketplace().getFrom('categories').then((response) => {
		...
		fetchMetadata: async function() {
			if(Object.keys(this.metadata).length) {
				...
				await ZESTEXAPI().marketplace().getFrom('metadata').then((response) => {
