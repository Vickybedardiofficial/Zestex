    ...
    export default defineComponent({
        setup: function() {
            ...
            const openSwitcher = async (data) => {
				...
				await ZESTEXAPI().userSettings().getFrom('account/linked').then((response) => {
            ...
            onMounted(() => {
                ZESTEXEventBus.on('account-switcher:open', openSwitcher);
            });

            onUnmounted(() => {
                ZESTEXEventBus.off('account-switcher:open', openSwitcher);
            });
