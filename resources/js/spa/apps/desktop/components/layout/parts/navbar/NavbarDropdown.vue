<template>
    <div class="relative cursor-pointer leading-none">
        <span
            class="sidenav-inactive flex items-center text-lab-pr2"
            v-on:click.stop="state.mainMenu.toggle"
        >
            <span class="size-icon-normal shrink-0">
                <SvgIcon name="dots-horizontal" type="solid"></SvgIcon>
            </span>
            <span class="text-par-l ml-2.5">
                {{ $t('labels.more') }}
            </span>
        </span>

        <div v-if="state.mainMenu.status" v-on:click.stop="state.mainMenu.close" v-outside-click="state.mainMenu.close" class="absolute bottom-full mb-2 left-6 z-[120]">
            <DropdownMenu>
