		...
		setup: function(props, context) {
			...
			const fetchMentionedUsers = async function(username) {
				if(! state.isSearching) {
					...
					await ZESTEXAPI().autocompletes().with({
						query: username
			...
			onMounted(() => {
				ZESTEXEventBus.on('editor:mention-input', fetchMentionedUsers);
			});

			onUnmounted(() => {
				ZESTEXEventBus.off('editor:mention-input', fetchMentionedUsers);
			});
