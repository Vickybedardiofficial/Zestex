        ...
        <div class="mb-12">
            <a href="{{ route('business.dashboard.index') }}" class="flex items-center gap-2">
                ...
                <span class="font-bold text-lab-pr">
                    {{ $accountData?->name ?: me()->name }}
                </span>
                @if($accountData && $accountData->updated_at && $accountData->is_reviewed && $accountData->verified)
                    <span class="text-green-900 font-semibold">&check;</span>
                @endif
            </a>
        </div>
        <x-sidebar.navlist>
