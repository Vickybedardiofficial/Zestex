...
@section('pageContent')
    <div class="mb-6">
        <div class="mb-4 flex items-center justify-between gap-3">
            ...
            <a href="{{ route('business.ads.create') }}" class="btn btn--primary">
                {{ __('business/ads.create_new_ad') }}
            </a>
        </div>

        <div id="ads-list" class="mt-4">
            <x-tabs.tabs>
    ...
    @if($adsList->isNotEmpty())
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            @foreach ($adsList as $adData)
                @include('business::ads.parts.index.ad-card', ['adData' => $adData])
            @endforeach
        </div>
    @else
        @if($type == 'all')
            <x-page-states.empty title="{{ __('business/ads.empty_state.index_all.title') }}" desc="{{ __('business/ads.empty_state.index_all.desc') }}"></x-page-states.empty>
        @elseif($type == 'active')
            <x-page-states.empty title="{{ __('business/ads.empty_state.index_active.title') }}" desc="{{ __('business/ads.empty_state.index_active.desc') }}"></x-page-states.empty>
        @else
            <x-page-states.empty title="{{ __('business/ads.empty_state.index_archived.title') }}" desc="{{ __('business/ads.empty_state.index_archived.desc') }}"></x-page-states.empty>
        @endif
    @endif
